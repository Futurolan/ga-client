...
function NewsContent ({ data: { loading, error, node } }) {
  ...
  if (node) {
    const processedContent = node.content.processed.replace(new RegExp('/sites/default/files/inline-images/', 'g'), `${process.browser ? process.env.BACKEND_API_URL : ''}/sites/default/files/inline-images/`)
